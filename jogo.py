# jogo.py (Atualizado com Lógica de Buff 60/40)
...
def criar_tabuleiro(largura_tela, altura_tela):
    ...
    largura_total_tabuleiro = (cols * CASA_TAM) + ((cols - 1) * CASA_ESPACO)
    altura_total_tabuleiro = (rows * CASA_TAM) + ((rows - 1) * CASA_ESPACO)
    
    margem_x = (largura_tela - largura_total_tabuleiro) // 2
    margem_y = (altura_tela - altura_total_tabuleiro) // 2

    if margem_x < 0: margem_x = 20
    if margem_y < 0: margem_y = 20
...
# ----------------------------
# História do Jogo (Lore e Epílogo) 
# ----------------------------
...
def criar_jogadores(num_jogadores=2):
    ...
    for i in range(num_jogadores):
        peoes.append({
            ...
            "buff_ativo": False, # ## <<< MUDANÇA: Buff do Neo
        })
...
def resetar_partida():
    ...
    for p in peoes:
        ...
        # p["personagem_id"] = None # Mantém o personagem escolhido
        p["buff_ativo"] = False # ## <<< MUDANÇA: Reseta o Buff
    jogador_atual = 0
    pular_idx = None
...
# ## <<< MUDANÇA: Cartas separadas em Boas e Ruins
cartas_boas = [
...
cartas_ruins = [
    ...
    ("Smith te joga através de uma parede. 'É o som do inevitável.' O impacto te joga para trás. Volte 2 casas.", {"mov": -2}, "smith"), 
    ("O Merovíngio te intercepta. 'Causalidade. Ação e reação.' Ele te atrasa. Volte 3 casas.", {"mov": -3}, "merovingio"),
    ("Os Gêmeos do Merovíngio te encurralam. Você se esconde e espera eles passarem, perdendo terreno. Volte 3 casas.", {"mov": -3}, "gemeos"), 
    ...
    ("Agentes múltiplos convergem na sua posição. Você é forçado a recuar para se esconder. Volte 4 casas.", {"mov": -4}, "smith")
]
